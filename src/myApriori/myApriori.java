...
// Set中的对象不允许重复, TreeSet的特点就是内部元素有序

public class myApriori {

	private double minsup = 0.1;// 最小支持度
	private double minconf = 0.1;// 最小置信度
	// 注意使用IdentityHashMap，否则由于关联规则产生存在键值相同的会出现覆盖
	private IdentityHashMap ruleMap = new IdentityHashMap();
	// private String[] transSet = { "abc", "aee", "acd", "bce", "aaa", "esf", "ade", "ade" };// 事务集合，可以根据需要从构造函数里传入
	private String[] transSet = new String[500];
	...
	public myApriori() {
		...
		System.out.println("The length of the time series = " + transSet.length);
	...
	public myApriori(String[] transSet) {
	...
	public int counts() {// 初始化1候选集的大小

		String temp1 = null;
		char temp2 = 'a';
		// 遍历所有事务集String 加入集合，set自动去重了
		// System.out.println(transSet.length);
	...
	public void item1_gen() {// produce the Frequency 1_itemset
	...
	public double count_sup(String x) {// count the support of the Candidate 1_itemset

		int temp = 0;

		for (int i = 0; i < transSet.length; i++) {
			for (int j = 0; j < x.length(); j++) {
				if (transSet[i].indexOf(x.charAt(j)) == -1) // Only count the first repeated substring
					break;
	...
	public void canditate_gen(int k) {// generate the candidate
	...
	public void maxfrequent_gen() {
		...
		iterator = maxFrequency.iterator();

		while (iterator.hasNext()) {
			System.out.println("maxFrequency[] : " + iterator.next());
		}
	...
	public void rulePrint() {
		...
		while (iterator.hasNext()) {
			...
			// x = toDigit(x);
			// y = toDigit(y);
//			System.out.println(x + (x.length() < 5 ? "\t" : "") + "-->" + y + "\t" + count_sup(x + y) + "\t" + temp);
			System.out.println(mapTable(x) + (x.length() < 5 ? "\t" : "") + "-->" + mapTable(y) + "\t" + count_sup(x + y) + "\t" + temp);
	...
	public void subGen(String s) {
		...
		for (int i = 1; i < (1 << s.length()) - 1; i++) {
			...
			System.out.println("subGen() : " + "x = " + x + ", y = " + y + ", x + y = " + x + y + count_sup(x + y) + " " + count_sup(x));
	 ...
	 * 2016 2016年10月10日 下午1:48:22 Max
	 * String
	 * TODO
	 */
	public String mapTable(String x) {
